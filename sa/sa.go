...
func (ssa *SQLStorageAuthority) statusForOrder(ctx context.Context, order *corepb.Order) (string, error) {
	...
	// Keep a count of the valid authorizations seen
	validAuthzs := 0
	// Loop over each of the order's authorization objects to examine the authz status
	for _, authz := range authzs {
		// If **any** of the authorizations are invalid, the order is invalid
		if authz.Status == core.StatusInvalid {
			return string(core.StatusInvalid), nil
		}

		// If **any** of the authorizations are pending, the order is pending
		if authz.Status == core.StatusPending {
			return string(core.StatusPending), nil
		}

		// If **any** of the authorizations are deactivated, the order is deactivated
		// TODO(@cpu): Should an order be deactivated in this case? Unclear.
		if authz.Status == core.StatusDeactivated {
			return string(core.StatusDeactivated), nil
		}

		// If the authz is valid, increment the validAuthz count and continue the loop
	...
	// The loop above checks for any invalid or pending authzs and returns early.
	// Somehow we made it this far but also don't have the correct number of valid
	// authzs.
	if !fullyAuthorized {
		return "", berrors.InternalServerError(
			"Order %d has the incorrect number of valid authorizations & no pending, "+
				"deactivated or invalid authorizations", *order.Id)
	}
