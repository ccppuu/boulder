...
// TODO(@cpu): Comment performRemoteValidation func
...
// TODO(@cpu): Comment processRemoteResults func
func (va *ValidationAuthorityImpl) processRemoteResults(
	...
	/*
		TODO(@cpu): Restore (and move?) this somewhere...
		defer func() {
...
// TODO(@cpu): Comment logRemoteValidationDifferentials func
func (va *ValidationAuthorityImpl) logRemoteValidationDifferentials(
	...
	if err != nil {
		...
		va.log.Warningf("Could not marshal log object in "+
			"logRemoteValidationDifferentials: %v", err)
	}

	va.log.Infof("remoteVADifferentials JSON=%q", string(logJSON))
}
...
func (va *ValidationAuthorityImpl) PerformValidation(ctx context.Context, domain string, challenge core.Challenge, authz core.Authorization) ([]core.ValidationRecord, error) {
	...
	if prob != nil {
		...
		prob = va.processRemoteResults(domain, prob, remoteErrors)
		if features.Enabled(features.EnforceMultiVA) {
			if prob != nil {
				challenge.Status = core.StatusInvalid
				challenge.Error = prob
				logEvent.Error = prob.Error()
