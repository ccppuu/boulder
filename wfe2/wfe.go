...
func (wfe *WebFrontEndImpl) NewRegistration(ctx context.Context, logEvent *requestEvent, response http.ResponseWriter, request *http.Request) {
	...
	// We use extractJWK rather than lookupJWK as the key extractor for verifyPOST
	// here because by definition a NewAccount request does not have an associated
	// account to read the key from and must use an embedded JWK.
